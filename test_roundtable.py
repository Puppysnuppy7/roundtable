...
class RoundtableTests(unittest.TestCase):
    ...
    def test_bugsend_flag_invokes_installer_main_with_bugsend(self):
        with mock.patch("install.main", return_value=0) as mock_install_main, \
             mock.patch("sys.argv", ["roundtable", "--bugsend", "--message", "it broke"]):
            exit_code = roundtable.main()
            self.assertEqual(exit_code, 0)
            mock_install_main.assert_called_once_with(
                ["--bugsend", "--message", "it broke"])

    def test_bugsend_flag_forwards_message_and_log(self):
        with tempfile.TemporaryDirectory() as td:
            log_path = Path(td) / "custom.log"
            with mock.patch("install.main", return_value=0) as mock_install_main, \
                 mock.patch("sys.argv", ["roundtable", "--bugsend", "-m", "it broke",
                                        "--log", str(log_path)]):
                exit_code = roundtable.main()
                self.assertEqual(exit_code, 0)
                mock_install_main.assert_called_once_with(
                    ["--bugsend", "--message", "it broke", "--log", str(log_path)])

    def test_bugsend_flag_without_message_or_log_forwards_bare(self):
        with mock.patch("install.main", return_value=0) as mock_install_main, \
             mock.patch("sys.argv", ["roundtable", "--bugsend"]):
            exit_code = roundtable.main()
            self.assertEqual(exit_code, 0)
            mock_install_main.assert_called_once_with(["--bugsend"])
